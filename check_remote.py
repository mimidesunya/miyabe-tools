...
host_image_dir = '~/services/miyabe-tools/data/reiki/kawasaki/images'
container_image_dir = '/var/www/data/reiki/kawasaki/images'
host_source_html = '~/services/miyabe-tools/data/reiki/kawasaki/source/H309999991001A_j.html'
public_image_url = 'http://localhost:8301/data/reiki/kawasaki/images/S-20100304-011Z0001.gif'
public_source_html_url = 'http://localhost:8301/data/reiki/kawasaki/source/H309999991001A_j.html'
...
result = ssh_exec(config, f'cd ~/services/miyabe-tools && docker compose exec web ls -lh {container_image_dir}/S-20100304-011Z0001.gif 2>&1')
print(result)

print("\n=== Count images in Docker container ===")
result = ssh_exec(config, f'cd ~/services/miyabe-tools && docker compose exec web sh -c "ls {container_image_dir}/*.gif 2>/dev/null | wc -l"')
...
result = ssh_exec(config, f'cd ~/services/miyabe-tools && docker compose exec web find {container_image_dir} -name "*.gif" | wc -l')
...
result = ssh_exec(config, f'cd ~/services/miyabe-tools && docker compose exec web ls -lh {container_image_dir}/S-20100304-011Z0001.gif')
print(result)
...
result = ssh_exec(config, f'cd ~/services/miyabe-tools && docker compose exec web cat {container_image_dir}/S-20100304-011Z0001.gif | wc -c')
