...
`data/local_assembly_minutes_urls.tsv` は、全国自治体マスタ (`data/japan_local_governments.tsv`) に対応する地方議会会議録URL一覧です。
...
- `App Mints` で拾えなかった自治体は `data/local_government_homepages.csv` を起点に再探索
...
```powershell
pwsh -File tools/municipalities/build_minutes_urls_tsv.ps1
```

必要に応じて `-HomepageCsv data/local_government_homepages.csv` を明示できます。
