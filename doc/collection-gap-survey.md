...
### 1-3-3. 突き合わせた結果: 確証のある取りこぼしは 4 自治体

dbsr（172）と gijiroku.com（83）で突き合わせた結果:

| 系統 | 本会議のみ | → 取りこぼし | → 取得元も本会議のみ |
| --- | ---: | ---: | ---: |
| dbsr | 58 | **4** | 54 |
| gijiroku.com | 13 | **0** | 13 |
...
いずれも 1-4 の修正で取れるようになるはずで、取り直しを実行中（6 章）。

**当初の「835 自治体（55%）が取りこぼし」は、実際には 5 自治体だった**
（上の 4 件＋福岡県）。kaigiroku.net（221）と kensakusystem（65）は突き合わせ待ちだが、
kensakusystem はツリーを全部歩く作りなので取りこぼしは起きにくく、
実際に米子市・鳴門市・沖縄市・佐賀市は取得元が本会議しか公開していなかった。

### 1-3-4. gijiroku.com の旧形式で会議種別を決め打ちしていた（**対応済み**）
...
1. `lib/homepage/runtime.php`（「反映待ち」表示）と `docker/mcp/src/server.ts`
   （例規検索の既定ソート）は**サーバーへ反映していない**。デプロイが要る。
   MCP はコンテナの作り直しも要る
2. 今回の修正は**サーバーへ個別にファイルを置いただけで、git には入っていない**。
   別の作業ツリーからデプロイすると `rsync --delete` で巻き戻る。
   コミットしておかないと壊れやすい
