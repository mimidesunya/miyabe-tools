...
if ($method === 'POST') {
    ...
    if (!$user) { poster_boards_require_login(); /* リダイレクトされる */ }
    ...
    $pdo = poster_boards_open_tasks_pdo($slug);
    try {
        $uid = poster_boards_upsert_user($pdo, $user);
    } catch (Throwable $e) {
        http_response_code(500);
    ...
    if ($action === 'set_status') {
        ...
        // 権限チェック: 位置調整は管理者のみ許可
        $allowOffset = poster_boards_is_admin(poster_boards_current_user());
        if (!$allowOffset) {
            http_response_code(403);
            echo json_encode(['error' => '位置調整は管理者のみ許可されています'], JSON_UNESCAPED_UNICODE);
            exit;
        }
        ...
        if ($action === 'update_info') {
            ...
            $boardsPdo = poster_boards_open_boards_pdo($slug);
