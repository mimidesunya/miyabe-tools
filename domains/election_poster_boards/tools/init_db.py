...
TSVファイルは dev/boards/data/{slug}/data.tsv に配置されている必要があります。

使用方法:
  python domains/election_poster_boards/tools/init_db.py <slug>
"""
...
def init_boards_db(slug: str, root: Path, tsv_file: Path) -> None:
    """boards.sqlite を初期化"""
    schema_path = Path(__file__).resolve().parent / "schema" / "boards.sql"
...
def init_tasks_db(slug: str, root: Path) -> None:
    """tasks.sqlite を初期化"""
    schema_path = Path(__file__).resolve().parent / "schema" / "tasks.sql"
    db_dir = root / "data" / "boards" / slug
...
def main() -> int:
    if len(sys.argv) < 2:
        print("使用法: python domains/election_poster_boards/tools/init_db.py <slug>")
        return 1

    slug = sys.argv[1]
    root = Path(__file__).resolve().parents[3]

    # TSVファイルのパスを固定: dev/boards/data/{slug}/data.tsv
    ...
    tsv_dir = root / "dev" / "boards" / "data" / slug
