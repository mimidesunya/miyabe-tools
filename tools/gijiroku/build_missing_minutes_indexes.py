...
def main() -> int:
    ...
    targets = list(gijiroku_targets.iter_gijiroku_targets(configured_only=args.configured_only))
    ...
    if args.list_targets:
        ...
        for target in targets:
            print(
                ...
                f"{target['name']}\t{target['downloads_dir']}"
            )
        return 0
    ...
    for index, target in enumerate(targets, start=1):
        print(
            f"[START {index}/{len(targets)}] {target['slug']} minutes.sqlite を点検します",
            flush=True,
        )
        try:
