...
```bash
pip install -r tools/kawasaki_council_minutes/requirements.txt
...
```bash
python tools/kawasaki_council_minutes/scrape_kawasaki_minutes.py \
  --ack-robots \
  --max-meetings 20 \
  --save-html
```
...
```bash
python tools/kawasaki_council_minutes/scrape_kawasaki_minutes.py --ack-robots
```
