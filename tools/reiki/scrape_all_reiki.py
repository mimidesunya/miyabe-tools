...
def record_busy_index_result(
    ...
    *,
    ...
    stdout_path.write_text(
        "別プロセスが検索インデックスを更新中のため、このバッチからのインデックス更新はスキップしました。\n",
        encoding="utf-8",
    )
...
def record_target_result(
    ...
    *,
    ...
    latest_freshness = (
        freshness_metadata.reiki_target_freshness(target)
...
def restrict_to_resume_targets_when_needed(targets: list[dict]) -> list[dict]:
    ...
    for target in targets:
        reason = reiki_priority.update_check_skip_reason(target)
        if reason:
...
def main() -> int:
    ...
    targets = restrict_to_resume_targets_when_needed(reiki_priority.sort_targets_by_priority(targets))
